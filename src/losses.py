...
class Contrastive(nn.Module):
    ...
    def forward(self, scores):
        if self.ltype == 'infonce':
            # Scaling logits，clamp 防止 logit_scale 过大导致溢出
...
class PushPullLoss(torch.nn.Module):
    ...
    def class_loss(self, outputs, target_classes):
        ...
        if pred_logits.numel() == 0:
            # 本 batch 没有前景匹配，跳过正例 loss
            pos_loss = torch.tensor(0.0, device=src_logits.device, requires_grad=True)
        elif self.class_ltype == 'BCE':
        ...
        if bg_logits.numel() == 0:
            # 本 batch 没有背景预测，跳过负例 loss
            neg_loss = torch.tensor(0.0, device=src_logits.device, requires_grad=True)
        else:
            neg_targets = torch.zeros(bg_logits.shape).to(bg_logits.device)
            neg_loss = F.mse_loss(bg_logits, neg_targets)

        return torch.nan_to_num(pos_loss, nan=0.0), torch.nan_to_num(neg_loss, nan=0.0)
    ...
    def loss_boxes(self, outputs, targets, indices, idx, num_boxes):
        ...
        loss_giou = torch.nan_to_num(loss_giou, nan=0.0, posinf=1.0)
    ...
    def forward(
        ...
        if lvis_pred_scores is not None:
            ...
            loss_rank = torch.tensor(0, dtype=torch.float32, device=predicted_boxes.device)
