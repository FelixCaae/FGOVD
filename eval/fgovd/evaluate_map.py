...
def apply_NMS(preds, iou=0.5):
    ...
    return preds
def evaluate_map_ddp(preds_list, test_set, device=None, evaluate_all_vocabulary=True, disable_nms=False, n_neg=5, simplify_errors=False, get_tensors=False):
    import os
    ...
    for imm in test_set['images']:
        #遍历test set中所有图像，提取annotation,然后提取对应的预测，使用metric更新
    ...
    # Compute the results
    print(f"Rank {dist.get_rank()}: 开始compute()")
    result = metric.compute()  # 卡住点
    print(f"Rank {dist.get_rank()}: compute()完成")  # 永远执行不到
    if get_tensors:
        return result
